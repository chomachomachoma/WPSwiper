...
//Scrollbar
Swiper.prototype.plugins.scrollbar=function(e,t){function s(e){if(document.querySelectorAll)return document.querySelectorAll(e);else return jQuery(e)}function T(){v.style.width="";v.style.height="";if(u){f=e.h.getWidth(a,true);c=e.width/e.h.getWidth(e.wrapper);h=c*(f/e.width);p=f*c;v.style.width=p+"px"}else{l=e.h.getHeight(a,true);c=e.height/e.h.getHeight(e.wrapper);h=c*(l/e.height);d=l*c;if(d>l)d=l;v.style.height=d+"px"}}var n=t&&t.container;if(!n)return;var r={hide:true,draggable:true,snapOnRelease:false};t=t||{};for(var i in r){if(!(i in t)){t[i]=r[i]}}if(!document.querySelectorAll){if(!window.jQuery)return}if(!t.container.nodeType){if(s(t.container).length==0)return}var o=t.container.nodeType?t.container:s(t.container)[0];var u=e.params.mode=="horizontal",a=o,f,l,c,h,p,d;var v=document.createElement("div");v.className="swiper-scrollbar-drag";if(t.draggable)v.className+=" swiper-scrollbar-cursor-drag";a.appendChild(v);if(t.hide)a.style.opacity=0;var m=e.touchEvents;if(t.draggable){var g=false;function b(t){g=true;if(t.preventDefault)t.preventDefault();else t.returnValue=false;x(t);clearTimeout(N);e.setTransition(a,0);a.style.opacity=1;e.setWrapperTransition(100);e.setTransition(v,100)}function w(t){if(!g)return;if(t.preventDefault)t.preventDefault();else t.returnValue=false;x(t);e.setWrapperTransition(0);e.setTransition(a,0);e.setTransition(v,0)}function E(n){g=false;if(t.hide){clearTimeout(N);N=setTimeout(function(){a.style.opacity=0;e.setTransition(a,400)},1e3)}if(t.snapOnRelease){e.swipeReset()}}var S=e.support.touch?a:document;e.h.addEventListener(a,m.touchStart,b,false);e.h.addEventListener(S,m.touchMove,w,false);e.h.addEventListener(S,m.touchEnd,E,false);function x(t){var n=y=0;var r;if(u){n=(t.pageX||t.clientX)-e.h.getOffset(a).left-p/2;if(n<0)n=0;else if(n+p>f){n=f-p}}else{y=(t.pageY||t.clientY)-e.h.getOffset(a).top-d/2;if(y<0)y=0;else if(y+d>l){y=l-d}}e.setTranslate(v,{x:n,y:y});var i=-n/h;var s=-y/h;e.setWrapperTranslate(i,s)}}var N;var C={onFirstInit:function(e){T()},onInit:function(e){T()},onTouchMoveEnd:function(n){if(t.hide){clearTimeout(N);a.style.opacity=1;e.setTransition(a,200)}},onTouchEnd:function(n){if(t.hide){clearTimeout(N);N=setTimeout(function(){a.style.opacity=0;e.setTransition(a,400)},1e3)}},onSetWrapperTransform:function(n){if(u){var r=n.x*h;var i=p;if(r>0){var s=r;r=0;i=p-s}else if(-r+p>f){i=f+r}e.setTranslate(v,{x:-r});v.style.width=i+"px"}else{var o=n.y*h;var c=d;if(o>0){var s=o;o=0;c=d-s}else if(-o+d>l){c=l+o}e.setTranslate(v,{y:-o});v.style.height=c+"px"}if(e.params.freeMode&&t.hide){clearTimeout(N);a.style.opacity=1;N=setTimeout(function(){a.style.opacity=0;e.setTransition(a,400)},1e3)}},onSetWrapperTransition:function(t){e.setTransition(v,t.duration)},onDestroy:function(){var t=e.support.touch?a:document;e.h.removeEventListener(a,m.touchStart,b,false);e.h.removeEventListener(t,m.touchMove,w,false);e.h.removeEventListener(t,m.touchEnd,E,false)}};return C}
